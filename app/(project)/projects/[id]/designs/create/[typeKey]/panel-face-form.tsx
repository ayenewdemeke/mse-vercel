...
export default function PanelFaceForm({ projectId, typeName, action }: Props) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="max-w-sm">
            ...
            <Input id="name" name="name" type="text" placeholder="e.g. Abutment Panel Check" className="mt-1" />
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <Field name="fc" label="f'c (ksi)" />
            <Field name="fy" label="fy (ksi)" />
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <Field name="lPanel" label="L panel (ft)" />
            <Field name="hPanel" label="h panel (ft)" />
            <Field name="tPanel" label="t panel (in)" />
            <Field name="ssr" label="S sr (in)" />
            <Field name="cCoverPos" label="Cover +ve (in)" />
            <Field name="cCoverNeg" label="Cover −ve (in)" />
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Field name="barNumVert" label="Vertical bar #" />
            <Field name="spacingVert" label="Vertical spacing (in)" />
            <Field name="barNumHor" label="Horizontal bar #" />
            <Field name="spacingHor" label="Horizontal spacing (in)" />
          </div>
        </div>

        {/* Loads from Internal Stability */}
        <div className="rounded-xl border bg-card p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field name="huStr" label="HU Strength I (kip/ft)" />
            <Field name="huEe" label="HU Extreme Event I (kip/ft)" />
          </div>
        </div>
